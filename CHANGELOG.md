...
- 增加 `stop.bat` / `python server.py --stop`：网页打不开或进程卡住时，只结束本项目总控台，不停止启动台里已经运行的应用。
- 设置中心「外观」提供四套界面：指挥台（黑白）以及暮紫、松绿、琥珀三套带颜色的外观；明暗（浅/深/自动）仍可单独选择。
...
- 新增「Ops 指挥台」为默认 UI 主题（深空蓝黑/雾灰双色、柔和圆角细边、蓝色强调），主题清单中固定排首位；保留 `#themeCss` 整包加载与 `uiTheme` 配置机制。
